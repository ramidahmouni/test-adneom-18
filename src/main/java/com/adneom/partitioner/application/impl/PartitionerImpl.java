...
public class PartitionerImpl implements Partitioner {
	 ...
	 */
	@Override
	public Partitions partition(List<String> strList, Integer partitionSize) {
	 ...
	 */
	@Override
	public List<Partitions> partition(Map<Integer, List<String>> mapPartitionSizeListStr) {
		...
		if(mapPartitionSizeListStr != null && !mapPartitionSizeListStr.isEmpty()) {
			...
			for(Map.Entry<Integer, List<String>> entry : mapPartitionSizeListStr.entrySet()) {
					...
					//calling unit method
					partitions = this.partition(strList, partitionSize);
